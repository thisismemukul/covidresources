...
if(isset($_POST['search'])){
    ...
    $state = $_POST['statesearch'];
        $result = $con->query("SELECT * FROM resources WHERE aproved ='Y' AND resources='$res' AND r_name ='$r_name' AND state_name ='$state' ORDER BY ID DESC");
        $results = $result->fetch_all(MYSQLI_ASSOC);
            if(($res == 'all')||($state == 'all')){
    $results = $con->query("SELECT * FROM resources WHERE aproved ='Y' ORDER BY ID DESC");
...
<body>
    ...
    <div id="upload" class="about_area">
        ...
        <div class="row mb-4">
                ...
                <select id='filterstate' style='border-radius: 8px;  padding: 2px 7px; display:inline-block;' name="statesearch" class="boxed_btn required" required>
								<option disabled selected>Select</option>
								<option value='all'>All</option>
                            ...
                            <button style='border-radius: 8px;  padding: 2px 7px; display:inline-block;' type="submit" name="search" class="boxed_btn">Submit</button>
                </form>
				</div>
...
Tweet</a>
<a class="twitter-hashtag-button"
  href="https://twitter.com/intent/tweet?button_hashtag=covid">
